###################################################
# FOLLOWING WERE WRITTEN BY MATTHEW A. SIMONSON   #
###################################################
##
...
SRT <- function(dataset,CORES,PERMS,clean.genelist,threshold,KB){ # START SRT UMBRELLA FUNCTION
  ...
  generate_snp_path <- function(dataset,clean.genelist,KB){ # This function should be ran first to generate the list of SNPs within pathways
 ...
 write.table(snp.paths,file=paste("",dataset,".SRT_paths.txt",sep=""),quote=FALSE,row.names=FALSE,col.names=FALSE,sep='\t') # Also write out with correct name
  ...
  for (i in 1:PERMS){ # generate permuted phenotypes
    ...
    write.table(assoc.phe.perm,file=paste(dataset,'.assoc_pheno',i,'',sep=""),row.names=FALSE,col.names=TRUE,quote=FALSE) # write out phenotype file for initial association to be used by Plink
  } # END LOOP i
...
# Convert all SNP ratios into z-values based on null of SNP ratio's for pathway (this standardizes all pathways)
get_zval<- function(ratio,null){
  zval <- (ratio-mean(null))/sd(null)
  return(zval)
  }
 
z.path.data <- path.data
names(z.path.data) <- c('PATH','Observed.Z')
for(i in 1:nrow(z.path.data)){
  SRs <- as.vector(path.data[i,3:ncol(z.path.data)],'numeric')
  SR.ob <- z.path.data[i,2]
  z.null <- sapply(SRs,get_zval,SRs)  
  z.path.data[i,3:ncol(z.path.data)] <- z.null # assign null values
  z.path.data[i,2] <- get_zval(SR.ob,SRs)
}

# Calculate uncorrected p-values from z-values: 

 get_pval <- function(ob,null){
  pval<- mean(null>ob)
  if(pval==0){
   pval <- 1/(length(null)) # less than lowest
  }
  return(pval)
  }
 ...
 for(i in 1:nrow(z.path.data)){
   uc.pvals[i] <- get_pval(z.path.data[i,2],z.path.data[i,3:ncol(z.path.data)])
 }
 
 # Find empirical corrected p-values:
 
 z.path.sorted <- apply(z.path.data[,3:ncol(z.path.data)],2,sort,decreasing=TRUE)
 null.multitest <- as.numeric(z.path.sorted[1,])
 c.pvals <- sapply(uc.pvals,get_pval,null.multitest) # store corrected p-values
 ...
 path.pvals$'P-VAL' <- uc.pvals
 path.pvals$'CORRECTED.P-VAL' <- c.pvals
...
dataset <- 'CARDIA.clean'
CORES <- 10
PERMS <- 1000
clean.genelist <- "clean.genelist"
threshold <- 0.05
KB <- 20
